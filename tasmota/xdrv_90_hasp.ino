...
#define D_PRFX_HASP "Hasp"
#define D_CMND_HASP_PAGE "Page"
#define D_CMND_HASP_CLEARPAGE "ClearPage"
#define D_CMND_HASP_DIM "Dim"
#define D_CMND_HASP_LIGHT "Light"
#define D_CMND_HASP_WAKEUP "WakeUp"
#define D_CMND_HASP_JSON "Json"
...
void TempDrawVLine(uint16_t x, uint16_t y, int16_t len, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_len = len;
...
void TempDrawLine(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_x2 = x2;
...
void TempDrawCircle(uint16_t x, uint16_t y, uint16_t rad, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_rad = rad;
...
void TempDrawFilledCircle(uint16_t x, uint16_t y, uint16_t rad, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_rad = rad;
...
void TempDrawRectangle(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_x2 = x2;
...
void TempDrawFilledRectangle(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2, uint16_t color)
{
  dsp_x = x;
  dsp_y = y;
  dsp_x2 = x2;
...
void TempDrawStringAt(uint16_t x, uint16_t y, char *str, uint16_t color, uint8_t flag)
{
  dsp_x = x;
  dsp_y = y;
  dsp_str = str;
...
void TempReAllocScreenBuffer(void)
{
  DisplayFreeScreenBuffer();
...
void TempReAllocLogBuffer(void)
{
  DisplayFreeLogBuffer();
...
{
  /*
  Response_P(PSTR("{\"" D_PRFX_HASP "\":{\"" D_CMND_HASP_MODEL "\":%d,\"" D_CMND_HASP_WIDTH "\":%d,\"" D_CMND_HASP_HEIGHT "\":%d,\""
    D_CMND_HASP_MODE "\":%d,\"" D_CMND_HASP_DIMMER "\":%d,\"" D_CMND_HASP_SIZE "\":%d,\"" D_CMND_HASP_FONT "\":%d,\""
    D_CMND_HASP_ROTATE "\":%d,\"" D_CMND_HASP_REFRESH "\":%d,\"" D_CMND_HASP_COLS "\":[%d,%d],\"" D_CMND_HASP_ROWS "\":%d}}"),
