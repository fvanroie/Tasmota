...
void dispatch_screenshot(const char *, const char *filename)
{
    ...
    else if (strlen(filename) > 31 || filename[0] != '/')
    { // Invalid filename
...
void dispatch_topic_payload(const char *topic, const char *payload)
{
    ...
    if (topic == strstr_P(topic, PSTR("command/")))
    { // startsWith command/
        topic += 8u;
...
#if HASP_USE_CONFIG > 0
    if (topic == strstr_P(topic, PSTR("config/")))
    { // startsWith command/
        topic += 7u;
...
{
    if ((eventid == HASP_EVENT_LONG) || (eventid == HASP_EVENT_HOLD))
        return; // don't send repeat events
...
void dispatch_backlight(const char *, const char *payload)
{
    // Set the current state
    if (strlen(payload) != 0)
        guiSetBacklight(is_true(payload));
